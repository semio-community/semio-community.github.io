...
function applyPreviewChrome() {
	...
	document.body.classList.add("relative", "min-h-screen", "w-full", "bg-surface");
}
		...
		<div className="relative m-auto flex min-h-screen w-full flex-1 max-w-6xl">
			<div className="relative m-auto w-full max-w-5xl grow">
				<div className="m-auto grid min-h-screen grid-rows-[1fr_auto] px-4 md:px-8">
					<main className={`relative grow ${mainClassName ?? ""}`}>{children}</main>
					<Footer />
				</div>
			</div>
		</div>
...
// biome-ignore lint/suspicious/noExplicitAny: Decap CMS runtime data is untyped
...
function toAssetUrl(getAsset: PreviewProps["getAsset"], value?: string) {
	...
	const rewriteLegacyPath = (p: string) => {
		...
		if (p.startsWith("/assets/")) return p.replace(/^\/assets\//, "/src/assets/");
		return p;
	};
...
const EventPreview: React.FC<PreviewProps> = ({ entry, getAsset }) => {
	...
	const gallery =
		data?.images?.gallery?.map((img: string) => toAssetUrl(getAsset, img)) || undefined;
...
const ResearchPreview: React.FC<PreviewProps> = ({ entry, getAsset }) => {
	...
	const organizationLinks: ResearchOrganizationLink[] = (data?.organizations || []).map(
...
const safeTemplate = (Component: React.FC<PreviewProps>) => (props: PreviewProps) => {
	try {

...
const hasCustomOgImage = (entry: { data: Record<string, unknown> }) =>
	// biome-ignore lint/suspicious/noExplicitAny: ogImage field is not typed in the shared data contract
...
async function loadOgImageAsset(
	...
	if (typeof meta === "string") {
		...
		const ref = meta.startsWith("__ASTRO_IMAGE_") ? meta.replace("__ASTRO_IMAGE_", "") : meta;
		...
		if (originalFile) {
			...
			if (candidates?.length && candidates.length > 0) {
				...
				// biome-ignore lint/style/noNonNullAssertion: candidate is set to a match above or remains the original value
				...
				// biome-ignore lint/style/noNonNullAssertion: cached is assigned above if it was undefined
				return { src: cached! };
			}
		}
		return undefined;
	...
	if (srcPath.startsWith("__ASTRO_IMAGE_")) {
		...
		try {
			...
			const raster = await toDataUrlWithRasterize(buffer, ext, meta.width, meta.height);
			return {
				src: raster.src,
				width: meta.width,
				height: meta.height,
	...
	// biome-ignore lint/style/noNonNullAssertion: dataUrl is assigned above before this return
...
async function mapHardware(
	...
	const status = entry.data.status ? getStatusLabel(entry.data.status) : undefined;
	const categoryLabel = getCategoryLabel("hardware", entry.data.category) || "Hardware";
	return {
...
async function mapSoftware(
	...
	return {
		...
		logoImage: await loadOgImageAsset(resolveContentLogo(entry.data.images), "software"),
		statusLabel: status,
		categoryLabel,
		statusTone: getStatusTone(entry.data.status),
		logoMode: "cover",
...
async function mapResearch(
	...
	const typeLabel = entry.data.type
		...
		: undefined;
	const logoImage = await loadOgImageAsset(resolveContentLogo(entry.data.images), "research");
	return {
...
async function mapEvent(
	...
	const locationParts = [entry.data.location?.city, entry.data.location?.country].filter(Boolean);
	const now = new Date();
	...
	return {
		...
		logoImage: await loadOgImageAsset(resolveContentLogo(entry.data.images), "events"),
		statusLabel,
		statusTone,
		categoryLabel: entry.data.type,
		listItems: [
			when ? { icon: "calendar", text: when } : null,
			locationParts.length ? { icon: "location", text: locationParts.join(", ") } : null,
...
async function mapOrganization(
	...
	return {
		...
		description: entry.data.collaborationSummary || entry.data.description || "",
		badgeIcon: createBadgeIcon(Buildings),
		avatarInitial: (entry.data.shortName || entry.data.name || "O").charAt(0).toUpperCase(),
		heroImage: await loadOgImageAsset(entry.data.images?.hero, "organizations"),
		logoImage:
			entry.data.images?.hero == null
				? await loadOgImageAsset(resolveContentLogo(entry.data.images), "organizations")
				: undefined,
...
export async function getStaticPaths() {
	...
	const pushPaths = (collection: keyof typeof builders, slug: string, data: OgImageProps) => {

	...
	return (
		<Section id={id} title={title} subtitle={subtitle} variant={variant}>
			<div className="max-w-6xl mx-auto">
				{items.length > 0 ? (
					<ExpandableGrid>{items.map((item) => renderCard(item))}</ExpandableGrid>
				) : (
...
function ExpandableGrid({
	...
	return (
		<div ref={sectionRef} data-expandable-section data-initial-count={initialCount.toString()}>
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" data-items-container>

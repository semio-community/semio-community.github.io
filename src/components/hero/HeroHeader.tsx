					...
					<div className="flex flex-wrap justify-center gap-4">
						{actions.map(
							(
								...
								<CallToActionButton
									// biome-ignore lint/suspicious/noArrayIndexKey: no stable key available for animation glyphs
									key={`${href}-${index}`}
									href={href}
									size="large"
									variant={variant}
...
function buildGlyphs(count: number, scaling: number): Glyph[] {
	...
	return Array.from({ length: count }, (_, idx) => {
		// biome-ignore lint/style/noNonNullAssertion: variants array is non-empty, index is always valid

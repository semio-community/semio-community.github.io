...
export const ContributorsSection: React.FC<ContributorsSectionProps> = ({
  ...
  if (!leadOrg && supportingOrgs.length === 0 && validOrgs.length === 0 && validPeople.length === 0) {
    return null;
  }
  ...
  return (
    <section className={`contributors-section mb-8 ${className}`}>
      <div className="bg-gradient-to-br from-surface-lighter to-surface rounded-xl border border-accent-one/20 p-6">
        ...
        {(categories.length > 0 || researchAreas.length > 0 || languages.length > 0) && (
          <div className="mt-6 pt-4 border-t border-accent-one/10 flex flex-wrap gap-2 text-xs text-color-600 dark:text-color-400">
            {categories.map((category, index) => (
              <ChipDisplay key={`cat-${index}`} text={category} variant="tertiary" />
            ))}
            {researchAreas.map((area, index) => (
              <ChipDisplay key={`area-${index}`} text={area} variant="default" />
            ))}
            {languages.map((lang, index) => (
              <ChipDisplay key={`lang-${index}`} text={lang} variant="default" />
            ))}
          </div>
        )}

        {/* Stats Row */}
        {showStats && (leadOrg || supportingOrgs.length > 0 || validPeople.length > 0) && (
          <div className="mt-6 pt-4 border-t border-accent-one/10 flex flex-wrap gap-6 text-xs text-color-600 dark:text-color-400">
            ...
            {validPeople.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <span>{validPeople.length} {validPeople.length === 1 ? 'person' : 'people'}</span>
              </div>
            )}
          </div>
        )}
...
const OrganizationChip: React.FC<{ organization: Contributor }> = ({ organization }) => {
  ...
  if (!data && !partnerId) {
    return (
      <span className="inline-flex items-center gap-2 px-2 py-1.5 bg-gray-100 dark:bg-gray-800 rounded-full">
        <svg className="w-5 h-5 text-gray-400" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 7V3H2v18h20V7H12zM6 19H4v-2h2v2zm0-4H4v-2h2v2zm0-4H4V9h2v2zm0-4H4V5h2v2zm4 12H8v-2h2v2zm0-4H8v-2h2v2zm0-4H8V9h2v2zm0-4H8V5h2v2zm10 12h-8v-2h2v-2h-2v-2h2v-2h-2V9h8v10zm-2-8h-2v2h2v-2zm0 4h-2v2h2v-2z"/>
        </svg>
        <span className="text-sm text-gray-500">Unknown Organization</span>
      </span>
    );
  }
  ...
  return (
    ...
    >
      {data?.logo ? (
        ...
        <svg className="w-6 h-6 text-accent-one group-hover:text-accent-two transition-colors" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 7V3H2v18h20V7H12zM6 19H4v-2h2v2zm0-4H4v-2h2v2zm0-4H4V9h2v2zm0-4H4V5h2v2zm4 12H8v-2h2v2zm0-4H8v-2h2v2zm0-4H8V9h2v2zm0-4H8V5h2v2zm10 12h-8v-2h2v-2h-2v-2h2v-2h-2V9h8v10zm-2-8h-2v2h2v-2zm0 4h-2v2h2v-2z"/>
        </svg>
      )}
      ...
      {role && (
        <span className="text-xs text-color-600 dark:text-color-400">• {role}</span>
      )}
    </a>
  );
};
...
const ChipDisplay: React.FC<{ text: string; variant?: 'default' | 'primary' | 'secondary' | 'tertiary' }> = ({
  text,
  variant = 'default'
}) => {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${variantClasses[variant]}`}>
      {text}
    </span>
  );
};

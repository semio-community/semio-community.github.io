...
export async function getUniqueLocations(): Promise<Array<{ city: string; country: string }>> {
	const partners = await loadOrganizationEntries();
	...
	const normalized = unique.map(({ city, country }) => ({
		// biome-ignore lint/style/noNonNullAssertion: city and country are filtered to be truthy above
		city: city!,
		// biome-ignore lint/style/noNonNullAssertion: city and country are filtered to be truthy above
		country: country!,
	}));
...
export async function getOrganizationCountByType(): Promise<Record<string, number>> {
	const partners = await loadOrganizationEntries();
...
export async function getOrganizationCountByCategory(): Promise<Record<string, number>> {
	const partners = await loadOrganizationEntries();
	return partners.reduce(
		(acc, partner) => {
...
export async function getOrganizationCountByCountry(): Promise<Record<string, number>> {
	const partners = await loadOrganizationEntries();
	return partners.reduce(
		(acc, partner) => {
...
export async function searchOrganizations(query: string): Promise<OrganizationEntry[]> {
	const partners = await loadOrganizationEntries();

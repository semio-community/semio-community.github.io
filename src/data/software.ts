...
export async function getSoftwareCountByCategory(): Promise<Record<string, number>> {
  const software = await getAllSoftware();
  return software.reduce((acc, item) => {
...
export async function getSoftwareCountByStatus(): Promise<Record<string, number>> {
  const software = await getAllSoftware();
  return software.reduce((acc, item) => {
...
export async function getSoftwareCountByLanguage(): Promise<Record<string, number>> {
  const software = await getAllSoftware();
...
export async function searchSoftware(query: string): Promise<CollectionEntry<"software">[]> {
  const software = await getAllSoftware();
  ...
  return software.filter(item => {
    return (
      ...
      item.data.language.some(lang => lang.toLowerCase().includes(lowerQuery)) ||
      item.data.platform.some(plat => plat.toLowerCase().includes(lowerQuery)) ||
      item.data.institutions.some(inst => inst.toLowerCase().includes(lowerQuery)) ||
      item.data.useCases.some(useCase => useCase.toLowerCase().includes(lowerQuery))
    );
  });
}
...
export async function getRelatedSoftware(
  ...
  const otherSoftware = allSoftware.filter(item => item.id !== currentItem.id);
...
export async function getSoftwareRequiringHardware(
  ...
  const software = await getAllSoftware();
  return software.filter(item =>
    item.data.requirements?.hardware?.some(
      hw => hw.toLowerCase().includes(hardwareName.toLowerCase())
    ) ?? false
  );
}
...
export async function filterSoftware(criteria: {
  ...
  if (criteria.category) {
    software = software.filter(item => item.data.category === criteria.category);
  }
  ...
  if (criteria.platform) {
    software = software.filter(item =>
      item.data.platform.some(plat =>
        plat.toLowerCase() === criteria.platform!.toLowerCase()
      )
    );
  }

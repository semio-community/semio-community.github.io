...
export async function getCurrentEvents(): Promise<CollectionEntry<"events">[]> {
	const events = await getAllEvents();
	const now = new Date();
	return events.filter((event) => event.data.startDate <= now && event.data.endDate >= now);
}

/** Get events filtered by type */
export async function getEventsByType(
	type: "conference" | "workshop" | "hackathon" | "meetup" | "webinar" | "competition",
	...
	return events.filter((event) => {
		if (location.city && location.country) {
			return (
				...
				event.data.location.country.toLowerCase() === location.country.toLowerCase()
			);
		}
		if (location.city) {
			return event.data.location.city.toLowerCase() === location.city.toLowerCase();
		}
		if (location.country) {
			return event.data.location.country.toLowerCase() === location.country.toLowerCase();
		}
		return false;
	});
}
...
export async function getEventsByDateRange(
	...
	return events.filter(
		(event) => event.data.startDate >= startDate && event.data.startDate <= endDate,
	);
}

/** Get events by year */
export async function getEventsByYear(year: number): Promise<CollectionEntry<"events">[]> {
	const events = await getAllEvents();
...
export async function getUniqueLocations(): Promise<{ city: string; country: string }[]> {
	...
	const locationStrings = events
		...
		.map((event) => `${event.data.location.city}|${event.data.location.country}`);
...
export async function searchEvents(query: string): Promise<CollectionEntry<"events">[]> {
	const events = await getAllEvents();
	...
	return events.filter((event) => {
		return (
			...
			event.data.topics.some((topic) => topic.toLowerCase().includes(lowerQuery)) ||
...
export async function getRelatedEvents(
	...
	const scored = otherEvents.map((event) => {
		...
		if (
			...
			score += 2;
		} else if (event.data.location.country === currentEvent.data.location.country) {
			score += 1;
		}
...
export async function groupEventsByMonth(
	...
	for (const key of Object.keys(grouped)) {
		...
		if (events) {
			events.sort((a, b) => a.data.startDate.getTime() - b.data.startDate.getTime());
		}
	}
	...
	if (criteria.location) {
		if (criteria.location.city) {
			events = events.filter(
				(event) =>
					event.data.location.city.toLowerCase() === criteria.location?.city?.toLowerCase(),
			);
		}
		if (criteria.location.country) {
			events = events.filter(
				(event) =>
					event.data.location.country.toLowerCase() === criteria.location?.country?.toLowerCase(),
			);
		}
	}
	...
	if (criteria.topics && criteria.topics.length > 0) {
		...
		events = events.filter((event) =>
			lowerTopics.some((topic) =>
				event.data.topics.some((eventTopic) => eventTopic.toLowerCase().includes(topic)),
			),
		);
	}

	if (criteria.dateFrom) {
		// biome-ignore lint/style/noNonNullAssertion: dateFrom is guaranteed by the surrounding if-check
		events = events.filter((event) => event.data.startDate >= criteria.dateFrom!);
	}

	if (criteria.dateTo) {
		// biome-ignore lint/style/noNonNullAssertion: dateTo is guaranteed by the surrounding if-check
...
export async function getEventsByRole(role: EventRole): Promise<CollectionEntry<"events">[]> {
	const events = await getAllEvents();
	return events.filter((event) => event.data.roles.includes(role));
}

/** Get next N upcoming events */
export async function getNextEvents(limit = 5): Promise<CollectionEntry<"events">[]> {

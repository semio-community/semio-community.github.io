...
function getContributorIds(entry: ResearchEntry): string[] {
	return (entry.data.contributors ?? []).map((contributor) => contributor.personId.toLowerCase());
}
...
export async function getResearchByYear(year: number): Promise<ResearchEntry[]> {
	const entries = await loadResearchEntries();
...
export async function getResearchByTopic(topic: string): Promise<ResearchEntry[]> {
	const entries = await loadResearchEntries();
...
export async function getResearchByContributor(personId: string): Promise<ResearchEntry[]> {
	const entries = await loadResearchEntries();
...
export async function getResearchByOrganization(organizationId: string): Promise<ResearchEntry[]> {
	const entries = await loadResearchEntries();
	const lower = organizationId.toLowerCase();
	return entries.filter((entry) =>
		(entry.data.organizations ?? []).some((org) => org.organizationId.toLowerCase() === lower),
	);
}

/** Get research entries by venue (conference or journal) */
export async function getResearchByVenue(venue: string): Promise<ResearchEntry[]> {
	const entries = await loadResearchEntries();
...
export async function getResearchByHardware(hardwareId: string): Promise<ResearchEntry[]> {
	const entries = await loadResearchEntries();
	const lower = hardwareId.toLowerCase();
	return entries.filter(
		(entry) => entry.data.relatedHardware?.some((hw) => hw.toLowerCase() === lower) ?? false,
	);
}

/** Get research entries related to specific software */
export async function getResearchBySoftware(softwareId: string): Promise<ResearchEntry[]> {
	const entries = await loadResearchEntries();
	const lower = softwareId.toLowerCase();
	return entries.filter(
		(entry) => entry.data.relatedSoftware?.some((sw) => sw.toLowerCase() === lower) ?? false,
	);
}

/** Get recent research entries sorted by publish date */
export async function getRecentResearch(limit = 10): Promise<ResearchEntry[]> {
	const entries = await loadResearchEntries();
	const withTime = entries
		...
		.filter((value): value is { entry: ResearchEntry; time: number } => value !== null)
...
> {
	...
	for (const entry of entries) {
		for (const contributor of entry.data.contributors ?? []) {
...
export async function searchResearch(query: string): Promise<ResearchEntry[]> {
	...
	return entries.filter((entry) => {
		...
		const descriptionMatch = entry.data.description?.toLowerCase().includes(lowerQuery) ?? false;
		...
		const hardwareMatch =
			entry.data.relatedHardware?.some((hw) => hw.toLowerCase().includes(lowerQuery)) ?? false;
		const softwareMatch =
			entry.data.relatedSoftware?.some((sw) => sw.toLowerCase().includes(lowerQuery)) ?? false;
...
export async function getRelatedResearch(
	...
	const currentOrganizations = new Set(
		(currentStudy.data.organizations ?? []).map((organization) => organization.organizationId),
	);
	...
	const scored = others.map((entry) => {
		...
		const sharedTopics = normalizeTopics(entry).filter((topic) => currentTopics.has(topic));
		score += sharedTopics.length * 3;

		const contributorIds = getContributorIds(entry);
		const sharedContributors = contributorIds.filter((id) => currentContributors.has(id));
		...
		const entryYear = getPublishYear(entry);
		if (currentYear !== null && entryYear !== null && Math.abs(entryYear - currentYear) <= 1) {
			score += 1;
		}
...
export async function filterResearch(criteria: {
	...
	if (criteria.yearFrom !== undefined) {
		entries = entries.filter((entry) => {
			const year = getPublishYear(entry);
			// biome-ignore lint/style/noNonNullAssertion: yearFrom is guaranteed by the surrounding if-check
	...
	if (criteria.yearTo !== undefined) {
		entries = entries.filter((entry) => {
			const year = getPublishYear(entry);
			// biome-ignore lint/style/noNonNullAssertion: yearTo is guaranteed by the surrounding if-check
	...
	if (criteria.contributors && criteria.contributors.length > 0) {
		...
		entries = entries.filter((entry) =>
			lowerContributors.some((personId) => getContributorIds(entry).includes(personId)),
		);
	}
	...
	if (criteria.relatedHardware && criteria.relatedHardware.length > 0) {
		...
		entries = entries.filter((entry) =>
			lowerHardware.some(
				(hardwareId) =>
					entry.data.relatedHardware?.some((hw) => hw.toLowerCase() === hardwareId) ?? false,
			),
		);
	}
	...
	if (criteria.relatedSoftware && criteria.relatedSoftware.length > 0) {
		...
		entries = entries.filter((entry) =>
			lowerSoftware.some(
				(softwareId) =>
					entry.data.relatedSoftware?.some((sw) => sw.toLowerCase() === softwareId) ?? false,
			),
		);
	}
...
export async function getResearchCoAuthorshipNetwork(): Promise<{
	...
	for (const entry of entries) {
		...
		for (const contributor of people) {
		...
		for (let i = 0; i < people.length; i++) {
			for (let j = i + 1; j < people.length; j++) {
				const key = [people[i]?.personId, people[j]?.personId].sort().join("|||");
	...
	const edges = Array.from(edgeMap.entries())
		...
		.filter((edge): edge is { source: string; target: string; weight: number } => edge !== null);

...
```bash
git clone https://github.com/semio-community/semio-community.github.io
cd semio-community.github.io
npm install
npm run dev    # dev server at http://localhost:4321
```

> **Note for Vizij contributors:** the Vizij repo lives in the `vizij-ai` GitHub org, not `semio-community`. The packages are still hosted in `semio-community`'s registry, so the same `read:packages` PAT on `npm.pkg.github.com` applies — no extra steps needed.
